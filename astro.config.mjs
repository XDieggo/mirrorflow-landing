...
const { site } = config;
const { title, logo, logo_darkmode, logo_text } = site;

export const locales = locals;

// Only use logo config if paths are not empty
const logoConfig = logo && logo.trim() !== '' ? {
  logo: {
    light: logo,
  ...
  integrations: [
    starlight({
      ...
      components: {
        ...
        SiteTitle: "./src/components/override-components/SiteTitle.astro",
      },
    }),
    sitemap({
      i18n: {
